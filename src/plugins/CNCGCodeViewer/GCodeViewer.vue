    ...
    <div ref="primarycontainer" class="primary-container mt-2 mb-5" v-resize="resize">
        <div :class="{ 'full-screen': fullscreen }" class="viewer-box">
            ...
            <div class="button-container" :class="{ 'button-container-drawer': drawer }">
                ...
                <v-btn small class="toggle-menu-button-close mr-2" @click="loadRunningJob" v-show="!(!isJobRunning || loading || visualizingCurrentJob)" :title="$t('plugins.gcodeViewer.loadCurrentJob.title')">
                   ...
                   <!--  <v-expansion-panel @click="scrollIntoView">
                    ...
                    <v-expansion-panel @click="scrollIntoView">
                        ...
                        <v-expansion-panel-content>
                            <v-card class="pa-2 mb-2">
								<v-card-title>
                                    {{$t('plugins.CNCGCodeViewer.viewerSettings')}}
								</v-card-title>
                                <v-card-text>{{$t('plugins.CNCGCodeViewer.background')}}</v-card-text>
                                <v-card-text class="">
                                    <gcodeviewer-color-picker :editcolor="backgroundColor" @updatecolor="value => updateBackground(value)"></gcodeviewer-color-picker>
                                </v-card-text>
                                <v-card-text>{{$t('plugins.CNCGCodeViewer.bedRenderMode')}}</v-card-text>
